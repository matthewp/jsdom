...
const reservedNames = ["annotation-xml",
  "color-profile",
  "font-face",
  "font-face-src",
  "font-face-uri",
...
// if `extends` is valid.
let tagInterfaceMap = {};
for(const interfaceName of Object.keys(tagMappings)) {
  const tags = tagMappings[interfaceName].tags;
  const iface = tagMappings[interfaceName].file.interface;

  for(const tagName of tags) {
    tagInterfaceMap[tagName] = iface;
  }
}
...
function isValidExtendsTag(tagName) {
  var iface = tagInterfaceMap[tagName];
...
exports.implementation = class CustomElementsRegistryImpl {
  ...
  define(name, constructor, options) {
    ...
    // 8.
    let extendss = options.extends || null;
    ...
    if(this._whenDefinedPromiseMap.has(name)) {
      let promise = this._whenDefinedPromiseMap.get(name);
      promise.resolve();
